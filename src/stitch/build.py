...
import stitch.util.oldshell as shell
...
def stitch(srcRoot, flags):
  """ run the stitch program to generate the build file. """

  initialCwd = os.getcwd()
  try:
    os.chdir(srcRoot)
    try:
      shell.sh("stitch", flags)
    except shell.CommandError, ce:
      raise BuildError("stitch exited with status " + str(ce.value))
  finally:
    os.chdir(initialCwd)
