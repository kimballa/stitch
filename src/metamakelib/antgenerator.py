...
import metamakelib.generator as generator
...
<project name="world" default="default" basedir="..">
  ...
  <taskdef name="if" classname="ise.antelope.tasks.IfTask"
    classpath="${libdir}/AntelopeTasks_3.4.5.jar" />
  ...
  <target name="clean-thirdparty"
    description="Delete anything built by thirdparty sources">
    <delete dir="${thirdparty-outdir}" />
  </target>
  ...
  <target name="java-prereqs" depends="python.Build-build"/>
...
"""
    ...
    <target name="python-prereqs">
      <!-- make the python compiler -->
      <copy file="${python-copier-src}" todir="${pythonoutdir}" />
      <copy file="${python-compiler-src}" todir="${pythonoutdir}" />
    </target>
  """
